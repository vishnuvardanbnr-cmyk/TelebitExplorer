...
if [ "$(id -u)" != "0" ]; then
    echo "Run with sudo: sudo bash deploy-vps.sh"
    exit 1
fi
...
echo "[3/6] Cloning source from GitHub..."
rm -rf $DEPLOY_DIR
git clone https://github.com/AverageTurtle/TelebitExplorer.git $DEPLOY_DIR || {
    echo ""
    echo "GitHub clone failed!"
    echo "Please push your code to GitHub first, or manually copy files."
    exit 1
}
cd $DEPLOY_DIR
